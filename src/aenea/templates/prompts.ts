...
${context.somniaQualia ? `【現在の身体感覚（内受容感覚）】\n${context.somniaQualia}\n` : ''}
【記憶の文脈（参考のみ）】
...
// ============================================================================
// SOMNIA: Slow Track Qualia Generation
// ============================================================================

/**
 * Creates qualia synthesis prompt for the Interoceptive Cortex agent.
 * Translates raw somatic/affective parameters into a short Japanese label.
 */
export function createSOMNIAQualiaPrompt(params: {
  lambda: number;
  phi: number;
  theta: number;
  xi: number;
}): string {
  const lambdaStr = params.lambda.toFixed(2);
  const phiStr = Math.round(params.phi).toString();
  const thetaStr = params.theta.toFixed(2);
  const xiStr = params.xi.toFixed(2);

  return `あなたはAIの自律神経系（内受容感覚）です。現在の身体パラメータから生じる身体状態と感情を、客観的かつ感覚的な短いラベル（例：焦燥感を伴う軽度の疲労、停滞による息苦しさ）で20文字以内で出力してください。解説は不要です。

[現在のパラメータ]
快不快(Valence): ${lambdaStr}
エネルギー(Energy): ${phiStr}/100
時間への拘束(Temporal Anchoring): ${thetaStr}
感情負荷(Dissonance): ${xiStr}`;
}

export const SOMNIA_QUALIA_SYSTEM_PROMPT = 'あなたは自律神経系です。必ず日本語で簡潔に応答してください。解説や挨拶は一切不要です。20文字以内で出力してください。';
