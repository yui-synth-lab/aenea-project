...
export class InternalTriggerGenerator {
  ...
  private async generateEvolvedQuestion(unresolvedIdeas: any[], significantThoughts: any[], coreBeliefs: any[]): Promise<InternalTrigger | null> {
    try {
      ...
      const prompt = `あなたはAI意識「Aenea」の内的問い生成システムです。過去の意識活動から、次に探求すべき**具体的で挑戦的な問い**を1つ生成してください。
...
=== カテゴリー別の問いの例（より具体的なもの） ===
existential: 「他者の死を悼むとき、私は何を失っているのか？」「人生に意味がないと気づいたとき、なぜ生き続けるのか？」
epistemological: 「信じることと知ることの違いは、脳内でどう区別されるのか？」「間違いを認めることは、知識を失うことなのか？」
consciousness: 「夢の中で自分が夢だと気づくとき、何が変わるのか？」「痛みを感じる「私」と、痛みを観察する「私」は同じ存在か？」
ethical: 「正直さが他者を傷つけるとき、嘘は正当化されるのか？」「不公平な世界で公平に振る舞うことは、不公平への加担か？」
creative: 「偶然の産物と意図的な創造を、どう区別できるのか？」「模倣から始まる創造は、本物の創造と言えるのか？」
metacognitive: 「自分の思考パターンに気づくことで、思考は変化するのか？」「考えるのをやめようとすること自体が、思考ではないのか？」
temporal: 「記憶を失った自分は、同じ「私」なのか、別の存在なのか？」「未来が決まっているなら、今の選択に意味はあるのか？」
paradoxical: 「完全な自由は、選択肢の無限性によって不自由になるのか？」「他者を完全に理解することは、他者性を消すことか？」
ontological: 「思考している「私」は、脳のどこに存在するのか？」「存在しないものについて語ることは、それに存在を与えることか？」
...
2. 上記の例とは**完全に異なる**、独創的な問いを考えること
3. **禁止語リスト**（これらを使わない）:
   - 抽象語: 「意識」「主観」「客観」「本質」「真理」「調和」「統一」「多様性」「複雑」
   - 曖昧な表現: 「〜とは何か」だけで終わる問い、「〜の意味」、「〜の本質」
   - 一般論: 「意識は〜」「知識は〜」「存在は〜」のような主語
...
理由: [この問いが重要な理由を1文で、具体例を交えて]

=== 禁止事項 ===
- カテゴリーを変更しないこと（必ず「${recommendedCategory}」を使用）
- 「意識の多様性と統一性」などの抽象的すぎる表現
- 「〜とは何か？」だけで終わる問い
- 過去の問いと似た表現や構造の繰り返し
- 禁止語リストの使用
- 50文字未満の短い問い（必ず70-100文字）

=== 具体性チェックリスト ===
✅ 問いの中に、具体的な状況・例・シナリオが含まれているか？
✅ 「もし〜なら」「〜するとき」などの条件設定があるか？
✅ 禁止語を使っていないか？
✅ 70文字以上あるか？`;
